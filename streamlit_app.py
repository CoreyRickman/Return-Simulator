...
st.title("📈 PPLI vs Taxable: Multi-Scenario Return Model (v1.3)")
...
with st.sidebar:
    ...
    r_stocks = r1.number_input("Stocks r% (global)", min_value=-50.0, max_value=50.0, value=7.0, step=0.1)
    r_bonds = r2.number_input("Bonds r% (global)", min_value=-50.0, max_value=50.0, value=4.0, step=0.1)
    r_alts = r3.number_input("Alternatives r% (global)", min_value=-50.0, max_value=50.0, value=6.0, step=0.1)
    ...
    st.subheader("PPLI")
    ppli_cost_pct = st.slider("PPLI Annual Cost % (global)", min_value=0.0, max_value=5.0, value=1.0, step=0.05)
...
# -----------------------------
# Results: Ranked table & growth chart
# -----------------------------
...
# Altair scatter with labels and point size by End Net
scatter = (
    alt.Chart(eff_df)
    .mark_circle(opacity=0.8)
    .encode(
        x=alt.X("Return:Q", axis=alt.Axis(format=".1%", title=x_title)),
        y=alt.Y("Tax Drag:Q", axis=alt.Axis(format=".1%", title=y_title)),
...
with st.expander("Modeling Notes & Assumptions"):
    st.markdown("""
- Version 1.3: Results are **ranked by Ending Net Value** (best to worst). Added a **Tax Efficiency** scatter: Return vs. Tax Drag, with point size = ending net value.
- Expected returns, tax mix, tax rates, PPLI cost, and strategy are global; scenarios set allocations, PPLI sleeve, and names.
- All calculations are annual and simplified.
    """)
